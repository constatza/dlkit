...
class TestImportRules:
    ...
    def test_test_isolation_patterns(self, test_files: List[Path]) -> None:
        """Ensure tests follow proper isolation patterns.

        This enforces good testing practices by ensuring tests use fixtures
        for data creation rather than creating data inside test methods.
        """
        violations = []
        ...
        # Note: This is a warning test, not a hard requirement
        if violations:
            pytest.skip(
                f"Tests should use fixtures for data creation. "
                f"Consider refactoring: {violations[:3]}..."  # Show first 3
            )
...
class TestArchitecturalCompliance:
    ...
    def test_solid_principles_documentation(self) -> None:
        ...
        # This is a placeholder for more sophisticated SOLID analysis
        # In a real implementation, you might analyze:
        # - Single Responsibility: class complexity metrics
        # - Open/Closed: extension points via protocols
        # - Liskov Substitution: subtype behavioral contracts
        # - Interface Segregation: interface size and cohesion
        # - Dependency Inversion: dependency direction analysis

        assert True, "SOLID principles analysis would go here"

    def test_factory_pattern_usage(self, project_root: Path) -> None:
        """Ensure factory patterns are used consistently."""
        factory_files = list((project_root / "src" / "dlkit").rglob("*factory*.py"))

        # Should have at least some factory files
        assert len(factory_files) > 0, "Factory pattern should be used in the architecture"
