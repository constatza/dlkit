...
class TestFlexibleDatasetWithNpz:
    ...
    def test_load_single_feature_from_single_array_npz(
        ...
        assert "data" in dataset.features
        assert dataset.features["data"].shape == (10, 5)
    ...
    def test_load_multiple_arrays_from_multi_npz(self, npz_multi_array: dict) -> None:
        ...
        assert len(dataset) == 10
        assert "features" in dataset.features
        assert "targets" in dataset.targets
        assert dataset.features["features"].shape == (10, 5)
        assert dataset.targets["targets"].shape == (10, 1)
    ...
    def test_load_multiple_features_from_same_npz(
        ...
        assert len(dataset) == 10
        assert "features" in dataset.features
        assert "latent" in dataset.features
        assert dataset.features["features"].shape == (10, 5)
        assert dataset.features["latent"].shape == (10, 3)
    ...
    def test_mixed_npy_and_npz_files(self, npz_multi_array: dict, tmp_path) -> None:
        ...
        assert len(dataset) == 10
        assert "features" in dataset.features
        assert "extra" in dataset.features
        assert dataset.features["features"].shape == (10, 5)
        assert dataset.features["extra"].shape == (10, 4)
    ...
    def test_npz_with_precision_context(self, npz_single_array: dict) -> None:
        ...
        with precision_override(PrecisionStrategy.FULL_64):
            dataset = FlexibleDataset(features=features)
            assert dataset.features["data"].dtype == torch.float64

        # Load with float32 precision
        with precision_override(PrecisionStrategy.FULL_32):
            dataset = FlexibleDataset(features=features)
            assert dataset.features["data"].dtype == torch.float32
    ...
    def test_npz_data_integrity(self, npz_multi_array: dict) -> None:
        ...
        loaded_data = dataset.features["features"].numpy()
    ...
    def test_entry_name_used_as_array_key(self, npz_multi_array: dict) -> None:
        ...
        assert dataset.features["features"].shape == (10, 5)
        np.testing.assert_allclose(
            dataset.features["features"].numpy(),
        ...
        assert dataset2.features["latent"].shape == (10, 3)
        np.testing.assert_allclose(
            dataset2.features["latent"].numpy(),
