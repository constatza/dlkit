...
> `coerce_root_dir_to_absolute`) now live in `dlkit.infrastructure.io.paths`.
> Tensor utilities (`ensure2d`) now live in `dlkit.engine.data.datasets.tensor_utils`.
> Checkpoint security now lives in `dlkit.engine.adapters.lightning.security`.
...
**Example**:
```python
from dlkit.infrastructure.utils.general import kwargs_compatible_with
...
**Example**:
```python
from dlkit.infrastructure.utils.system_utils import recommended_uvicorn_workers
...
## Related Modules
- `dlkit.infrastructure.io.paths`: Path utilities (`mkdir_for_local`, `normalize_user_path`, `coerce_root_dir_to_absolute`)
- `dlkit.engine.data.datasets.tensor_utils`: Tensor/dataloader helpers (`ensure2d`)
- `dlkit.engine.adapters.lightning.security`: Checkpoint security configuration
- `dlkit.domain.metrics.collect`: Metric value collection
- `dlkit.infrastructure.config`: Uses import utilities for dynamic component loading
- `dlkit.infrastructure.registry`: Uses import utilities for component resolution
