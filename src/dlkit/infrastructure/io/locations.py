...
the EnvironmentSettings root. No directories are created here.
"""
...
from dlkit.infrastructure.io.path_context import get_current_path_context, resolve_with_context
...
def root() -> Path:
    ...
    2) EnvironmentSettings.root_dir (from DLKIT_ROOT_DIR env var or SESSION.root_dir)
    ...
    # Check EnvironmentSettings fallback
    try:
        env_root = global_environment.get_root_path()
        if env_root != Path.cwd():
            logger.debug(f"Using root from EnvironmentSettings: {env_root}")
            return env_root
    except Exception:
        pass
