"""Path utility functions for local URI resolution and user-path normalization."""
...
def mkdir_for_local(uri: Url | str, *, root: Path | None = None) -> None:
    ...
    Args:
        uri: A local file URI or path string.
...
def normalize_user_path(value: str | Path | None, *, require_absolute: bool = False) -> Path | None:
    ...
    Args:
        ...
        require_absolute: If True, return None for non-absolute paths.
...
def coerce_root_dir_to_absolute(value: str | Path | None) -> Path | None:
    ...
    Returns:
        Absolute Path or None if value is None or not absolute.
    """
