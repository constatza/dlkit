...
> `dlkit.tools.config.data_entries`, not from this package.
...
import numpy as np
from dlkit.tools.io.sparse import save_sparse_pack, open_sparse_pack, validate_sparse_pack
...
```python
from dlkit.tools.io.sparse import PackFiles, save_sparse_pack, open_sparse_pack
...
`SparseFeature` is imported from `dlkit.tools.config.data_entries`.
