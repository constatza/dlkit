...
def train_mlflow(settings: Settings) -> None:
	...
	with mlflow.start_run(
		...
		signature = mlflow.models.infer_signature(datamodule.features, datamodule.targets)
		mlflow.log_input(mlflow_dataset, 'dataset')
		mlflow.pytorch.log_model(model, 'model', signature=signature)
		if settings.MLFLOW.client.register_model:
			mlflow.register_model(model_uri=f'runs:/{run_id}/model', name=settings.MODEL.name)
