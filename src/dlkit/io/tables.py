...
def read_table(file_path: str, **read_kwargs) -> pl.DataFrame:
	...
	if ext == '.csv':
		# Read CSV via Polars
		return pl.read_csv(file_path, **read_kwargs)  # :contentReference[oaicite:0]{index=0}
	elif ext in {'.parquet', '.pq'}:
		# Read Parquet via Polars
		return pl.read_parquet(file_path, **read_kwargs)  # :contentReference[oaicite:1]{index=1}
	else:
		raise ValueError(
