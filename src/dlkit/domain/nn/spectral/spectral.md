...
implement `from_entries(input_shapes, output_shapes, **kwargs)` for factory-compatible construction.
...
Raises `ValueError` if `num_layers < 1`.  `from_entries` is supported.
...
Raises `ValueError` if `num_layers < 2`.  `from_entries` is supported.
...
`FourierEnhancedFFNN` and `DualPathFFNN` expose `from_entries(input_shapes, output_shapes, **kwargs)`,
so the shared model factory can build them from dataset entry shapes.
