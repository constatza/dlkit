...
class ScaleEquivariantConstantWidthFactorizedFFNN(StandardEntryConsumer, ScaleEquivariantWrapper):
    ...
    **No projection layers.** Every one of the ``num_layers`` body layers is a
    ``FactorizedLinear``, including the last. There is no embedding at the input
    or regression at the output. For asymmetric inputs use
    :class:`~dlkit.domain.nn.ffnn.constrained.EmbeddedFactorizedFFNN`, whose
    embedding, body, and regression layers are all factorized.
