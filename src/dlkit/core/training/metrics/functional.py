...
def _compute_Av(matrix: Tensor, vector: Tensor) -> Tensor:
    ...
    Returns:
        (B, D) result of matrix-vector products.

    Raises:
        ValueError: If matrix is not a batched matrix tensor with shape (B, D, D).
    """
    if not isinstance(matrix, Tensor):
        raise ValueError(
            f"Expected matrix tensor with shape (B, D, D), got type {type(matrix).__name__}."
        )
    if matrix.dim() != 3:
        raise ValueError(
            f"Expected matrix with shape (B, D, D), got {tuple(matrix.shape)}. "
            "Callers must provide batched matrices (use (1, D, D) for shared matrices)."
        )
...
def compute_quadratic_form(vector: Tensor, matrix: Tensor) -> Tensor:
    ...
    Args:
        ...
        matrix: Positive (semi-)definite batched matrix.
    ...
    Examples:
        ...
        >>> A = torch.eye(2).unsqueeze(0)  # (1, 2, 2) shared via batch dim
        >>> compute_quadratic_form(v, A)  # tensor([1., 1.])
    """
    Av = _compute_Av(matrix, vector)
...
def compute_energy_norm(vector: Tensor, matrix: Tensor) -> Tensor:
    ...
    Examples:
        ...
        >>> A = torch.eye(2).unsqueeze(0)  # shared via batch dim
        >>> compute_energy_norm(v, A)  # tensor([5.])
    """
    return torch.sqrt(compute_quadratic_form(vector, matrix).clamp(min=0))


def _compute_relative_energy_components(
    preds: Tensor,
    target: Tensor,
    matrix: Tensor,
) -> tuple[Tensor, Tensor]:
    """Compute (error_norms, target_norms) for relative energy metrics in one call scope."""
    error_vectors = compute_error_vectors(preds, target)
    batch_vectors, dim = target.shape
    ...
    if matrix.layout == torch.strided:
        if batch_matrices == 1:
            dense = matrix.squeeze(0)
            error_Av = torch.mm(dense, error_vectors.T).T
            target_Av = torch.mm(dense, target.T).T
        elif batch_matrices == batch_vectors:
            error_Av = torch.bmm(matrix, error_vectors.unsqueeze(-1)).squeeze(-1)
            target_Av = torch.bmm(matrix, target.unsqueeze(-1)).squeeze(-1)
        else:
            raise ValueError(
                f"Batch mismatch: matrix batch={batch_matrices}, vector batch={batch_vectors}. "
                "Expected matrix batch to be 1 (shared) or equal to vector batch."
    ...
    target_norms = torch.sqrt((target * target_Av).sum(dim=-1).clamp(min=0))
    return error_norms, target_norms
...
def temporal_derivative_error(
    ...
    """
    squared_derivative = _temporal_derivative_update(
        preds,
        target,
        n=n,
...
def _temporal_derivative_compute(sum_squared_errors: Tensor, total: int) -> Tensor:
    ...
    """
    return compute_state_mean(sum_squared_errors, total)
...
def _absolute_vector_norm_compute(sum_norms: Tensor, total: int) -> Tensor:
    ...
    """
    return compute_state_mean(sum_norms, total)
